...
- **Still MOCK / not addressed by the perf pass:** the `select('*')` over-fetching in `lib/database/queries.ts`; catalog/detail client-side fetch waterfalls; `images.unoptimized: true` in `next.config.mjs` (all `next/image` unoptimized); no `revalidate`/cache on public pages; no search debounce in `course-grid.tsx`. These were catalogued in the diagnostic but NOT yet changed.
- **Polish batch (small, deferred):** `AdminHeader` still shows hardcoded "Admin User"/fake email/dead bell — wire it to `useUser()` like `InstructorHeader` (deliberately kept out of the context refactor to isolate scope).
